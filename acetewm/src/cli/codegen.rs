...
use std::path::Path;
use std::rc::Rc;
use crate::consts::{ATTRIBUTE_INLINE, ATTRIBUTE_NAME, ATTRIBUTE_ACE_REF, HTML_TAG_TO_TYPE, ATTRIBUTE_ACE_NAME, VALID_CUSTOM_ELEMENT_NAME, INVALID_CUSTOM_ELEMENT_NAME, ATTRIBUTE_TYPE, ATTRIBUTE_VALUE};
...
pub(crate) fn do_code_gen(file_path: &Path, base_name_hint: Option<&str>) -> Result<()> {
	...
	match base_name_hint {
		Some(template_name) => {
			file_path.push("autogen");
			fs::create_dir_all(&file_path)?;
			file_path.push(format!("{}.ts", template_name));
		},
		None => file_path.push("autogen.ts"),
	}
	...
	for node_ref in template_markup_root_elem.children() {
		...
		let template_class_name = regex_replace_all!(
		...
		for child_node_ref in node_ref.descendants() {
			...
			writeln!(
				file_handle,
				"\t\t\tthis._{} = this._element.shadowRoot!.querySelector(\"[ace-ref=\\\"{}\\\"]\")!;",
				ref_property_name,
				ref_raw_name,
			)?;
			if child_elem.name() == "form" {
		...
		// Write base autogen
		writeln!(file_handle, "export class {}Autogen extends HTMLElement {{", template_class_name)?;
